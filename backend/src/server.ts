...
// import { Product11 } from "./models/image";
const product11 = require('./models/image')
...
const MONGOURI='mongodb+srv://ecommerce1:0POnMk9RVqmKRQYb@ecommerce.mgtuunv.mongodb.net/'
...
app.post(
  ...
  AsynceHandler( async (req, res) => {
    ...
    if (!validatePassword) {
      res.status(400).send("please must be use one Special Character,one Aplphabet and One Number in password");
      return;
    }
...
app.post('/addproduct', upload.array('images'), async (req, res) => {
  try {
    const { name, description, price, MRP, off, origins, deliverTime, tag, variants,category } = req.body;

    // Parse the request body and create a new product instance
    const product = new product11({
      name,
      description,
      price,
      MRP,
...
app.get('/addproducts', async (req, res) => {
  ...
  try {
    ...
    const product = await product11.find(query);
    res.json(product);
  } catch (error) {
    console.error('Error fetching products:', error);
    res.status(500).json({ error: 'Internal server error' });
