...
async clearCart() {
  try {
    await this.dynamic.clearCart1();
    this.toaster.success('Cart items have been removed successfully.');
  } catch (error) {
    console.error('Error clearing cart:', error);
    this.toaster.error('Failed to clear cart items. Please try again later.');
  }
}
}
