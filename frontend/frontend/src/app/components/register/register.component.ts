   ...
   
   {
    ...
    passwordStrengthValidator(): ValidatorFn {
      return (control: AbstractControl): ValidationErrors | null => {
        ...
        const regex = /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[^\w\d\s])[A-Za-z\d@$!%*?&]{8,}$/;
...
if (this.registerForm.invalid) {
  ...
  if (this.registerForm.invalid) {
    ...
    Swal.fire('Error', 'Password must contain at least one special character, one lowercase letter, one uppercase letter, and one number.', 'error');
  }
...
this.user.register(user).subscribe(
  ...
  () => {
    ...
    Swal.fire({
      title: 'Error',
      text: 'Password must contain at least one special character, one lowercase letter, one uppercase letter, and one number.',
      icon: 'error',
